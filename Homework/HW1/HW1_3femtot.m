...
% fill global matrix
  ...
  for irow=1:n_unknowns
      ...
      if (irow == 1)
   deltaR = x(2) - x(1);
   Z(irow,irow) = -1./deltaR^2 + k0^2 * epsilon(irow)/3 - 1j*k0/deltaR;
   Z(irow,irow+1) = 1./deltaR^2 + k0^2 * epsilon(irow)/6;
          
      elseif(irow == n_unknowns)
   deltaL = x(irow) - x(irow-1);
   Z(irow,irow-1) = 1./deltaL^2 + k0^2 * epsilon(irow-1)/6;
   Z(irow,irow) = -1./deltaL^2 + k0^2 * epsilon(irow-1)/3 - 1j*k0/deltaL;
          
      else
   deltaR = x(irow+1) - x(irow);
   deltaL = x(irow) - x(irow-1);
   Z(irow,irow-1) = 1./deltaL^2 + k0^2 * epsilon(irow-1)/6;
   Z(irow,irow) = -2./deltaL^2 + k0^2 * (epsilon(irow-1) + epsilon(irow))/3;
   Z(irow,irow+1) = 1./deltaR^2 + k0^2 * epsilon(irow)/6;
      end
 end
