function coaxmeshvec

% Generate triangular-cell model of coaxial cross section
%
% Adapted from July 30, 2018 scalar FEM code 
%
% Code generates edge-related pointers for vector FEM
%
% A. F. Peterson November 8, 2018
%
% initialization
...
%   reorder edges so that boundary edges are at the end of the list
%
%   change 'pedton' and 'pedtop' to reflect new edge numbering

   ninner(nedgs)=0;
   nbound(nedgs)=0;
   nonbnd=nnods-ninnerbnd-nouterbnd;
   pedtonb(nedgs,2)=0;
   pedtopb(nedgs,2)=0;
   pedtoni(nedgs,2)=0;
   pedtopi(nedgs,2)=0;
   ...
   for ii=1:nedgs
       n1=pedton(ii,1);
       n2=pedton(ii,2);
       if((n1 > nonbnd) && (n2 > nonbnd))
           %  edge on boundary
           nbnd=nbnd+1;
           nbound(nbnd)=ii;
           pedtonb(nbnd,1)=n1;
           pedtonb(nbnd,2)=n2;
           pedtopb(nbnd,1)=pedtop(ii,1);
           pedtopb(nbnd,2)=pedtop(ii,2);
       else
           ...
           pedtoni(ninn,1)=n1;
           pedtoni(ninn,2)=n2;
           pedtopi(ninn,1)=pedtop(ii,1);
           pedtopi(ninn,2)=pedtop(ii,2);
       end
   end
 
   for ii=1:ninn
       pedton(ii,1)=pedtoni(ii,1);
       pedton(ii,2)=pedtoni(ii,2);
       pedtop(ii,1)=pedtopi(ii,1);
       pedtop(ii,2)=pedtopi(ii,2);
   end
   for ii=1:nbnd
       pedton(ii+ninn,1)=pedtonb(ii,1);
       pedton(ii+ninn,2)=pedtonb(ii,2);
       pedtop(ii+ninn,1)=pedtopb(ii,1);
       pedtop(ii+ninn,2)=pedtopb(ii,2);
   end
...
%        find all occurrances of patch 'i' in pointer 'pedtop'
         ...
         if(iedg ~= 3)
          disp('error in patch to edge connectivity');
         end
       end
...
%--------------------------------------------------------------------
%  part nine -- dump relevant data to file
%--------------------------------------------------------------------
